...
//const char* ssid = "Zyxel_BC81";
//const char* password = "e21604754@#";
...
double droneCurLocation[2] = {0, 0}; // Define the drone starting location
...
void loop() {
  ...
  if(flightPathMode){
    ...
    Serial.println("Drone Location:" + String(droneCurLocation[0]) + "," + String(droneCurLocation[1]));
    ...
    byte nearestNodeI = findNearestNode(droneCurLocation[0], droneCurLocation[1], locations, numNodes);
    Serial.print("Nearest Node: ");
    Serial.println(device_ids[nearestNodeI]);
    // Print the ordered device IDs for verification
    Serial.println("Ordered Device IDs:");
    for (int i = 0; i < numNodes; ++i)
    {
      device_ids[i] = orderedDeviceIds[i] + 1;
...
void sendMessage(String outgoing, byte MasterNode, byte otherNode) {
  digitalWrite(LORA_CS, LOW);
  digitalWrite(SD_CS, HIGH);
  Serial.print("sending lora to Node ");
