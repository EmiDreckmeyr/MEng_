...
bool RequestDataMode, locationCollectionMode, dataCollectionMode, rssiCollectionMode, webserverMode  = false;
int dataCount, rssiCount = 0;
String locas[numNodes][3];
String rssis[30][4];
String data[60][9];
...
void loop() {
  ...
  if(RequestDataMode){
    ...
    if (locationCollectionMode) {
      ...
      // receive LoRA -> interrupt in lora interrupt manage data received
    }
  ...
  if(webserverMode){
    ...
    events.send(getRSSIData().c_str(),"new_readings1" ,millis());
...
void storeLocations(String messages){
  ...
  int i = 0;
  while(sscanf(messages.c_str(), "{Node%d;%[^;];%[^}]}", &nodeId, &longitudeStr, &latitudeStr) == 3){
    locas[i][0] = String(nodeId);
    locas[i][1] = longitudeStr;
    locas[i][2] = latitudeStr;
    i++;
...
void storeData(String messages){
  ...
  String date, time;
  while(sscanf(messages.c_str(), "{Node%d;%d;%d-%d-%d;%d:%d:%d;%f;%f;%f;%f;%d}\n", &nId, &rID, &YY, &MM, &DD, &HH, &M, &SS, &Temp, &Pres, &Hum, &Lux, &SM) == 13){
...
void storeRssi(String messages){
  ...
  while(sscanf(messages.c_str(), "{Node%d;%d-%d-%d;%d:%d:%d;%d}", &nodeId, &Yy, &Mm, &Dd, &Hh, &Ms, &Ss, &rssi) == 8){
    // Store the values in the array
...
void sendMessage(String outgoing, byte MasterNode, byte otherNode) {
  Serial.println("sending request to Drone" + outgoing);
